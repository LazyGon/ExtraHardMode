...
/**
 * A MonsterGrinder Inhibitor which disables drops for Monsters which appear to
 * be farmed or which have been killed in conditions where the Player had a
 * clear advantage
 */
public class AntiGrinder extends ListenerModule {
    ...
    public boolean handleEntitySpawn(CreatureSpawnEvent event) {
        ...
        final boolean inhibitMonsterGrindersEnabled = CFG.getBoolean(RootNode.INHIBIT_MONSTER_GRINDERS,
                world.getName());
                ...
                case VILLAGE_INVASION: {
                    ...
                    switch (environment) {
                        ...
                        case NETHER:
                            ...
                            break;
                        case THE_END:
                            if (underBlockType != Material.END_STONE && underBlockType != Material.OBSIDIAN
                                    && underBlockType != Material.AIR/* dragon */) {
                                return false;
                            }
                            break;
                        default:
    ...
    public boolean handleEntityDeath(EntityDeathEvent event) {
        ...
        // FEATURE: monsters which take environmental damage or spawn from spawners
        // don't drop loot and exp (monster grinder inhibitor)
        if (inhibitMonsterGrindersEnabled && entity instanceof Monster) {
            if (EntityHelper.isLootLess(entity)) {
                ...
                clearDrops(event);
                return false;
            } else { // Evaluate if this kill was a too easy kill
                switch (entity.getType()) {
                    ...
                    default: {
                        // no loot for monsters which die standing in water, to make building grinders
                        // even more difficult
                        ...
                        if (killer != null) {
                            ...
                            Location[] locations = new Location[] {
                                    new Location(monsterEyeLocation.getWorld(),
                                            0.2 * monsterEyeLocation.getX() + 0.8 * playerEyeLocation.getX(),
                                            monsterEyeLocation.getY(),
                                            0.2 * monsterEyeLocation.getZ() + 0.8 * playerEyeLocation.getZ()),
                                    new Location(monsterEyeLocation.getWorld(),
                                            0.5 * monsterEyeLocation.getX() + 0.5 * playerEyeLocation.getX(),
                                            monsterEyeLocation.getY(),
                                            0.5 * monsterEyeLocation.getZ() + 0.5 * playerEyeLocation.getZ()),
                                    new Location(monsterEyeLocation.getWorld(),
                            ...
                            for (Location middleLocation : locations) {
                                ...
                                // monster doesn't have room above to hurdle a foot level block, unable to
                                // advance toward killer
                                else {
                                    ...
                                    if (top.getType() != Material.AIR &&
                                            ...
                                            // Since this feature seems to cause issues anyways, I'm gonna do a lazy
