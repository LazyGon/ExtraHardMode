 ...
 * access and storage of configuration option
 * values.
 *
 * @author Mitsugaru
 */
...
public abstract class ModularConfig extends EHMModule {
    ...
    protected void updateOption(final ConfigNode node, final ConfigurationSection config) {
        switch (node.getVarType()) {
            ...
            case COLOR: {
                ChatColor color = SpecialParsers.parseColor(config.getString(node.getPath(),
                        node.getDefaultValue() instanceof String ? (String) node.getDefaultValue() : ""));
    ...
    /**
     * Check the bounds on the parameters to make sure that all config variables are
     * legal and usable by the plugin.
     */
