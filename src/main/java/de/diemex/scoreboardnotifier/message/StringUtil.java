    ...
    public static List<MsgLineHolder> getLines(String message, ChatColor lineColor, String linePrefix, String suffix) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            // Line already has some words
                ...
                // If appending the current word would make line exceed the maximum length go to
                // next line
                if (words[i].length() + line.length() > maxLineLength - 1) // account for the space between words
                {
                    // If there is only 2 empty chars available or the word that would have been cut
                    // is shorter than two chars, rather create a new line
                    if (line.length() >= maxLineLength - 2 || line.length() + words[i].length() < maxLineLength + 2
                            && words[i].length() < 5 && line.length() > maxLineLength - 2) {
                        lines.add(line);
