...
public class ConfigPlotter {
    ...
    private void createPlot() {
        ...
        try {
            ...
            // final int percent = CFG.getEnabledWorlds().length > 0 ?
            // (plugin.getServer().getWorlds().size() * 100 / CFG.getEnabledWorlds().length)
            // : 0;
            ...
            for (final RootNode node : RootNode.values()) {
                switch (node) {
                    ...
                    // Please add future config nodes here, the bstats site defaults IDs to
                    // lowercase and does not allow editing of existing charts, nor easy bulk input
                    // of new ones...
                    // case SOMETHING_ELSE:
                    // {
                    // final int metricsVal = getMetricsValue(node);
                    ...
                    // }
                    //
                    // metrics.addCustomChart(new Metrics.SimplePie(node.toString().toLowerCase(),
                    // () -> result));
                    // break;
                    // }
                }
