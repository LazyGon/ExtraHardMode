 ...
 * <p/>
 * less health/food on respawn , loss of some of their inventory , enhanced
 * environmental damage , catching the player
...
public class Players extends ListenerModule {
    ...
    /**
     * Armor weight task
     * Using a runnable per-player instead of one runnable to iterate through all
     * players
     * allows us to be somewhat more compatible with other plugins that change
     * player speed,
    ...
    public void onPlayerRespawn(PlayerRespawnEvent respawnEvent) {
        ...
        if (respawnFood < 20 && respawnHealthPercentage > 0 && respawnHealthPercentage < 100) {
            ...
            SetPlayerHealthAndFoodTask task = new SetPlayerHealthAndFoodTask(player,
                    (int) player.getMaxHealth() * respawnHealthPercentage / 100, respawnFood);
            plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, task, 10L); // half-second delay
        }
        // FEATURE: players can't swim when they're carrying a lot of weight, reset the
        // cached value
    ...
    public void onEntityDeath(PlayerDeathEvent event) {
        ...
        final List<Material> blacklisted = CFG.getStringListAsMaterialList(RootNode.PLAYER_DEATH_ITEMS_BLACKLIST,
                world.getName());
        final List<Material> toolIds = CFG.getStringListAsMaterialList(RootNode.PLAYER_DEATH_TOOLS_LIST,
                world.getName());
                ...
                switch (cause) {
                    ...
                    case FALL:
                        applyEffectOnDmg(event, CFG.getPotionEffect(RootNode.ENHANCED_DMG_FALL, world.getName()),
                                CFG.getDouble(RootNode.ENHANCED_DMG_FALL_MULT, world.getName()));
                        // player.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, (int) (20
                        // * event.getDamage()), 4));
                    ...
                    case SUFFOCATION:
                        ...
                        break;
                    case LAVA:
                        applyEffectOnDmg(event, CFG.getPotionEffect(RootNode.ENHANCED_DMG_LAVA, world.getName()),
                                CFG.getDouble(RootNode.ENHANCED_DMG_LAVA_MULT, world.getName()));
                        // event.setDamage(event.getDamage() * 2);
                        break;
                    case FIRE_TICK:
                        applyEffectOnDmg(event, CFG.getPotionEffect(RootNode.ENHANCED_DMG_BURN, world.getName()),
                                CFG.getDouble(RootNode.ENHANCED_DMG_BURN_MULT, world.getName()));
                        // player.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 20, 1));
                        break;
                    case DROWNING:
                        applyEffectOnDmg(event, CFG.getPotionEffect(RootNode.ENHANCED_DMG_DROWNING, world.getName()),
                                CFG.getDouble(RootNode.ENHANCED_DMG_DROWNING_MULT, world.getName()));
                        break;
                    case STARVATION:
                        applyEffectOnDmg(event, CFG.getPotionEffect(RootNode.ENHANCED_DMG_STARVATION, world.getName()),
                                CFG.getDouble(RootNode.ENHANCED_DMG_STARVATION_MULT, world.getName()));
                        break;
    ...
    void onPlayerInteract(PlayerInteractEvent event) {
        ...
        final boolean extinguishingFireIgnites = CFG.getBoolean(RootNode.EXTINGUISHING_FIRE_IGNITES_PLAYERS,
                world.getName());
    ...
    // {
    // Player player = event.getPlayer();
    // if (!CFG.getBoolean(RootNode.ARMOR_SLOWDOWN_ENABLE,
    // player.getWorld().getName()))
    // return;
    // final int slowdownPercent = CFG.getInt(RootNode.ARMOR_JUMP_SLOWDOWN_PERCENT,
    // player.getWorld().getName());
    // final float armorPoints = PlayerModule.getArmorPoints(player);
    // if (player.getGameMode() != GameMode.CREATIVE && event.getTo().getY() >
    // event.getFrom().getY())
    // {
    // Block block, control;
    // Vector dir = player.getVelocity();
    // float armorPointsNorm = armorPoints / 0.8F;
    // float factor = (1 - armorPointsNorm * (slowdownPercent / 100F)) / 5; //for
    // every jump 5 move events are called
    ...
    // && !mapGet(player.getUniqueId(), false))
    // {
