...
/**
 * Parses a given StringList into a Map containing the block ids and
 * metadatavalues if existing The supported input can
 * be formatted like this
 * <p/>
 * 13:1,2,3 | COBBLESTONE.2.3.4 | 13:3.4.5
 * <p/>
 * The first number/string is always expected to be the string everything after
 * that can be seperated by any symbol
 * which isn't a number/character
 * <p/>
 * So 13:1@4%3 -> [13]{1,3,4} 13 is the blockid and 1,3,4 is the
 * blockmetadatavalues which are allowed
 *
 * @author Max
 */
public class SpecialParsers {
    ...
    public static ChatColor parseColor(String input) {
        ...
        try {
            ...
            // color = ChatColor.getByChar(input.replaceAll("[^0-9a-f]", "").substring(0,
            // 1)); //TODO test
        }
        return color;
     ...
     *         NEEDS_TO_BE_ADJUSTED = input not valid but has been corrected and can
     *         be written back to config
     */
    // public static Response<Map<Integer/*block id*/, List<Byte>>>
    // parseMaterials(List<String> stringList)
    // {
    // Status status = Status.OK;
    //
    // Map<Integer, List<Byte>> myFallingBlocks = new HashMap<Integer,
    // List<Byte>>();
    //
    // for (String blockString : stringList)
    // {
    // /* Use Pattern because it's 25% faster than String.replaceAll() */
    //// Pattern whitespace = Pattern.compile("\\s"); //Includes tabs/newline
    // characters
    ...
    // material =
    // Material.matchMaterial(onlyLetters.matcher(blockId).replaceAll(""));
    // }
    // /* we identified the block, but we should save it correctly so we don't have
    // to do it again */
    // if (material != null)
    // {
    // status = Status.NEEDS_TO_BE_ADJUSTED;
    ...
    // : "0";
    // int blockNumber = material != null ? material.getId() :
    // Integer.parseInt(onlyNums);
    //
    ...
    // }
    //
    // if (blockNumber != 0) //AIR can't fall and 0 will be default if parsing
    // failed
    ...
    /**
     * Turn a given Representation of Blocks and their metaData into a human
     * readable form
     *
     * @return List to be written to config
     */
    // public static List<String> convertToStringList(Map<Integer, List<Byte>>
    // blocksWithMeta)
    // {
