...
public class MsgModule extends EHMModule {
        ...
        switch (messages.getCat(node)) {
            case NOTIFICATION:
                if (player == null) {
                    ...
                    PlayerData playerData = plugin.getModuleForClass(DataStoreModule.class)
    ...
    public void send(Player player, MessageNode message, FindAndReplace... fars) {
        ...
        for (FindAndReplace far : fars) { /* Replace the placeholder with the actual value */
    ...
    /**
     * Send the player an informative message to explain what he's doing wrong. Play
     * an optional sound aswell
     * <p/>
     *
