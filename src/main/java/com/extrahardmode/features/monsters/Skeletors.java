...
public class Skeletors extends ListenerModule {
        ...
        if (event.getEntity() instanceof LivingEntity entity) {
            ...
            if (event.getDamager() instanceof Projectile bullet && event.getEntity() instanceof Player) {
                ...
                if (bullet.hasMetadata(key_knockbackArrow)) {
                    ...
                    int ticksBefore = entity.getFireTicks() >= 100 ? entity.getFireTicks() - 100 : 0; // fireticks from
                    // the arrow are
                    // already applied
        ...
        if (entity instanceof Skeleton && deflectPercent > 0) {
            ...
            if (damageSource instanceof Arrow arrow) {
                ...
                if (!skeliEvent.isCancelled()) {
                    ...
                    // teleport the arrow a single block farther along its flight path, note that .6
                    // and 12 were the unexplained recommended values, for speed and spread,
                    // reflectively, in the bukkit wiki
                    arrow.remove();
                    entity.getWorld().spawnArrow(arrow.getLocation().add((arrow.getVelocity().normalize()).multiply(2)),
                            arrow.getVelocity(), 0.6f, 12.0f);
                }
            }
        }
    }
    ...
    public void onShootProjectile(ProjectileLaunchEvent event) {
        ...
        final boolean explosionArrowEnable =
                CFG.getBoolean(RootNode.SKELETONS_FIREBALL_ENABLE, world.getName());// CFG.getBoolean(RootNode.);
        ...
        if (event.getEntity() instanceof Arrow arrow && event.getEntity().getShooter() instanceof Skeleton skeleton) {
            ...
            // Explosion arrows
                ...
                // Silverfish
            } else if (skeleton.getTarget() instanceof Player && silverfishEnable
                    && OurRandom.percentChance(silverfishPercent)) // To prevent tons of Silverfish
            {
                ...
                if (getMinionsSpawnedBySkeli(skeleton, plugin).size() < currentLimit
                    ...
                    // replace with silverfish, quarter velocity of arrow, wants to attack same
                    // target as skeleton
                    Creature silverFish = (Creature) skeleton.getWorld()
                            .spawnEntity(skeleton.getLocation().add(0.0, 1.5, 0.0), EntityType.SILVERFISH);
    ...
    public void onSilverfishSpawn(CreatureSpawnEvent event) {
        final boolean tempFix = CFG.getBoolean(RootNode.SILVERFISH_TEMP_POTION_EFFECT_FIX,
    ...
    public void onSkeletonDeath(EntityDeathEvent event) {
        final boolean removeSilverfish = CFG.getBoolean(RootNode.SKELETONS_RELEASE_SILVERFISH_KILL,
                ...
                if (isMinion(silverfish)) {
                    for (UUID id : getMinionsSpawnedBySkeli(event.getEntity(), plugin)) {
                        if (silverfish.getUniqueId() == id) {
                            // silverfish.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE,
                            // Integer.MAX_VALUE, 1));
     ...
     * @return Unique ids of all minions that have been spawned (minions can be
     * dead)
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * When an Entity spawns: Spawn a Skeleton sometimes instead of a EnderMan in
     * the end.
     *
