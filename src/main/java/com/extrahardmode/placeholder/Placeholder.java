    ...
    public String onPlaceholderRequest(Player player, String identifier) {
        ...
        if (identifier.equals("weight")) {
            ...
            if (playerData.cachedWeightStatus <= 0) {
                ...
                float inventoryPoints = (float) CFG.getDouble(RootNode.NO_SWIMMING_IN_ARMOR_INV_POINTS,
                        world.getName());
                float toolPoints = (float) CFG.getDouble(RootNode.NO_SWIMMING_IN_ARMOR_TOOL_POINTS, world.getName());
                playerData.cachedWeightStatus = PlayerModule.inventoryWeight(player, armorPoints, inventoryPoints,
                        toolPoints);
            }
