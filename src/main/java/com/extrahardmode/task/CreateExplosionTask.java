...
/**
 * Creates Explosions. The type determines the power, if there should be fire
 * and the blockDmg. The size of the
 * explosion is determined by the y-level. There are basically 2 settings for
 * every explosion, below and above the
 * specified y-level.
 * <p/>
...
public class CreateExplosionTask implements Runnable {
     ...
     * @param type     Type that determines size and possible blockdamage or fire of
     *                 explosion.
     */
    ...
    void createExplosion(Location loc, ExplosionType type) {
        ...
        // RoboMWM - we no longer provide "compatibility" for block logging plugins -
        // they can listen to BlockExplodeEvent
         ...
         * plugin.getModuleForClass(ExplosionCompatStorage.class).queueExplosion(
         * location, explosionCause);
         */

        // entity should be ignored so our code doesn't think that it's a regular
        // creeper etc.
     ...
     *             The Location
     * @param type
     *             ExplosionType determining the size of the Explosion and size of
     *             the area to check
     */
    /*
    ...
    /**
     * Check pillars in the 4 outer corners of a virtual cube around the explosion.
     * If the protection plugin uses a
     * cuboid selection system then there is no way that an explosion can reach into
     * a protected area without atleast on
     * corner touching it
     */
    /*
     * private ArrayList<Block> getBlockList(Location loc, int boomSize)
     * {
     * //Doesn't aim to be accurate, just to prevent explosions on the edges of
     * protected land
     ...
     * (x+n-1)/n == x/n but always rounds up
     * for (int j = -boomSize; j < boomSize; j += (boomSize + 4) / 5)
     * {
     * switch (i)
     * {
     * case 0:
     * cubeLoc = new Location(loc.getWorld(), loc.getX() + (double) boomSize,
     * loc.getY() + (double) j, loc.getZ() + (double) boomSize);
     * break;
     * case 1:
     * cubeLoc = new Location(loc.getWorld(), loc.getX() - (double) boomSize,
     * loc.getY() + (double) j, loc.getZ() + (double) boomSize);
     ...
     * loc.getY() + (double) j, loc.getZ() - (double) boomSize);
     * break;
     * case 4: //Locations in the middle inbetween the corners. Needed if explosion
     * bigger than claim
     * cubeLoc = new Location(loc.getWorld(), loc.getX() - (double) (boomSize / 2),
     * loc.getY() + (double) j, loc.getZ() - (double) (boomSize / 2));
     * break;
     * case 5:
     * cubeLoc = new Location(loc.getWorld(), loc.getX() - (double) (boomSize / 2),
     * loc.getY() + (double) j, loc.getZ() - (double) (boomSize / 2));
