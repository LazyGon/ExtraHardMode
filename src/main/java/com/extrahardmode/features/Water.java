...
public class Water extends ListenerModule {
    ...
    void onPlayerMove(PlayerMoveEvent event) {
        ...
        final boolean blockWaterElevators = CFG.getBoolean(RootNode.NO_SWIMMING_IN_ARMOR_BLOCK_ELEVATORS,
                world.getName());
        ...
        // FEATURE: no swimming while heavy, only enabled worlds, players without bypass
        // permission and not in creative
        if (noSwimingInArmor && !playerBypasses) {
            ...
            if (to.getY() > from.getY()) {
                ...
                if (fromBlock.getType() == Material.WATER && toBlock.getType() == Material.WATER
                        && underFromBlock.getType() == Material.WATER
                        && underFromBlock.getRelative(BlockFace.DOWN).getType() == Material.WATER) {
                    // if no cached value, calculate
                    if (playerData.cachedWeightStatus <= 0) {
                        playerData.cachedWeightStatus = PlayerModule.inventoryWeight(player, armorPoints,
                                inventoryPoints, toolPoints);
                    }
                    // if too heavy let player feel the weight by pulling them down, if in boat can
                    // always swim
                    if (playerData.cachedWeightStatus > maxWeight && !player.isInsideVehicle()) {
                        drown(player, drowningRate, overEncumbranceExtra, playerData.cachedWeightStatus, maxWeight,
                                normalDrownVel, overwaterDrownVel);
                    }
                }
                // when you swim up waterfalls and basically are flying with only a tip of your
                // body in water
                else if (blockWaterElevators && !playerModule.isPlayerOnLadder(player) && !player.isInsideVehicle()
                        && !player.isFlying()) {
                    if (playerData.cachedWeightStatus <= 0) {
                        playerData.cachedWeightStatus = PlayerModule.inventoryWeight(player, armorPoints,
                                inventoryPoints, toolPoints);
