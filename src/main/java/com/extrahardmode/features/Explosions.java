...
public class Explosions extends ListenerModule {
        ...
        // GHASTS
            ...
            if (customGhastExplosion) {
                ...
                if (fireball.getShooter() instanceof Ghast) {
                    ...
                    new CreateExplosionTask(plugin, sourceEntity.getLocation(), ExplosionType.GHAST_FIREBALL,
                            sourceEntity).run();
                }
            }
        }
    ...
    // RoboMWM - remove "compatibility" (see
    // com.extrahardmode.module.ExplosionCompatStorage for more details)
    /*
     * /**
     * Provide compatibility for block protection and logging plugins.
     * <pre>
     * 1. call world.createExplosion()
     * 2. save responsible entity and location with {@link
     * com.extrahardmode.module.ExplosionCompatStorage}
     * --> our code starts here
     * 3. compare location and call an additional {@link
     ...
     * {
     * ExplosionCompatStorage explosionStorage =
     * plugin.getModuleForClass(ExplosionCompatStorage.class);
     ...
     * if (savedLoc.getBlockX() == eventLoc.getBlockX() && savedLoc.getBlockY() ==
     * eventLoc.getBlockY() && savedLoc.getBlockZ() == eventLoc.getBlockZ())
     * {
     * // There is no way for us to pass the actual cause (the entity) of an
     * explosion to bukkit other than this additional event
     * FakeEntityExplodeEvent compatEvent = new
        ...
        if (event.getEntity() instanceof FallingBlock) {
            ...
            if (fallBaby.hasMetadata(tag)) {
                ...
                if (obj instanceof Location) {
                    ...
                    // If close place the block as if the player broke it first: stone -> cobble,
                    // gras -> dirt etc.
                    else {
    ...
    /**
     * This method only gets called by us once we have evaluated that we are allowed
     * to trigger an explosion
     *
     * @param event event that occurred
     */
    public void explosionLogic(EntityExplodeEvent event) {
        Validate.notNull(event.getEntity(),
                "Entity was null [explosionLogic], but shouldn't be! x:" + event.getLocation().getBlockX() + " y:"
        ...
        // final boolean flyingBlocks =
        // CFG.getBoolean(RootNode.EXPLOSIONS_FYLING_BLOCKS_ENABLE, world.getName());

        final boolean turnStoneToCobble = CFG.getBoolean(RootNode.EXPLOSIONS_TURN_STONE_TO_COBLE, world.getName());

        // final boolean flyOtherPlugins =
        // CFG.getBoolean(RootNode.EXPLOSIONS_FYLING_BLOCKS_ENABLE_OTHER, worldName);
        // final int flyPercentage =
        // CFG.getInt(RootNode.EXPLOSIONS_FLYING_BLOCKS_PERCENTAGE, worldName);
        // final double upVel = CFG.getDouble(RootNode.EXPLOSIONS_FLYING_BLOCKS_UP_VEL,
        // worldName);
        // final double spreadVel =
        // CFG.getDouble(RootNode.EXPLOSIONS_FLYING_BLOCKS_SPREAD_VEL, worldName);
    ...
    // | _ \ __| | | / __| /_\ | _ ) | | __| | __| | | | \| |/ __|_ _|_ _/ _ \| \| /
    // __| \ \ \ \
    // | / _|| |_| \__ \/ _ \| _ \ |__| _| | _|| |_| | .` | (__ | | | | (_) | .` \__
    // \ > > > >
    // |_|_\___|\___/|___/_/ \_\___/____|___| |_| \___/|_|\_|\___| |_|
    // |___\___/|_|\_|___/ /_/_/_/
    //

    /**
     * Creates 4 additional explosions around a location to make the shape of the
     * crater random and more natural
     *
