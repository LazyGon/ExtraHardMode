...
public class Validation {
                    ...
                    switch (node.getSubType()) {
                        ...
                        default:
                            throw new UnsupportedOperationException(
                                    "SubType of " + node.getPath() + " doesn't have a validation method");
                    }
                }
            } else {
    ...
    private static void throwWrongTypeError(ConfigNode node, ConfigNode.VarType type) {
        throw new IllegalArgumentException("Expected a ConfigNode with Type " + type.name() + " but got "
                + node.getVarType().name() + " for " + node.getPath());
    }
}
